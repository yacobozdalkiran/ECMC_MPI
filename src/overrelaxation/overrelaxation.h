...
namespace overrelaxation {
    ...
    void hit(GaugeField &field, const GeometryCB &geo, size_t site, int mu, SU3 &A);

    // Full overrelaxation sweep (sequential/single-thread)
    void sweep(GaugeField &field, const GeometryCB &geo);
}
...
namespace mpi::overrelaxationcb {
    ...
    void sweep(GaugeField &field, const GeometryCB &geo, site_parity update_parity, int N_hits);

    // Runs a specified number of overrelaxation sweeps using checkerboard updates
    void full_sweep(GaugeField &field, const GeometryCB &geo, int N_hits);
}
