...
namespace mpi::ecmccb {
    ...
    std::pair<std::pair<size_t, int>,int> lift_improved(const GaugeField &field, const GeometryCB &geo, size_t site, int mu, int j,
    ...
    std::vector<double> samples_improved(GaugeField &field, const GeometryCB &geo, const ECMCParams &params, std::mt19937_64 &rng, HaloObs &halo_obs, mpi::MpiTopology &topo, parity active_parity);
}
