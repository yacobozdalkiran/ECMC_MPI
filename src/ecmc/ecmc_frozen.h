...
namespace ecmc_frozen {
    void compute_list_staples(const GaugeField &field, const GeometryFrozen &geo, size_t site, int mu, std::array<SU3,6> &list_staple);
    ...
    std::pair<std::pair<size_t, int>,int> lift_improved(const GaugeField &field, const GeometryFrozen &geo, size_t site, int mu, int j,
    ...
    size_t random_site(const GeometryFrozen &geo, std::mt19937_64 &rng);
    std::vector<double> samples_improved(GaugeField &field, const GeometryFrozen &geo, double beta, int N_samples,
