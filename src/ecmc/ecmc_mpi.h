...
namespace mpi::ecmc {
    void compute_list_staples(const GaugeField &field, const GeometryHaloECMC &geo, size_t site, int mu, std::array<SU3,6> &list_staple);
    ...
    std::pair<std::pair<size_t, int>,int> lift_improved(const GaugeField &field, const GeometryHaloECMC &geo, size_t site, int mu, int j,
    ...
    size_t random_site(const GeometryHaloECMC &geo, std::mt19937_64 &rng);
    std::vector<double> samples_improved(GaugeField &field, const GeometryHaloECMC &geo, const ECMCParams &params, std::mt19937_64 &rng, HaloObs &halo_obs, mpi::MpiTopology &topo);
}
